					...
					if backends[frontend.Backend] == nil {
						...
						case types.Drr:
							...
							for serverName, server := range configuration.Backends[frontend.Backend].Servers {
								...
								hcOpts := parseHealthCheckOptions(rebalancer, configuration.Backends[frontend.Backend].HealthCheck)
								if hcOpts != nil {
						...
						case types.Wrr:
							...
							hcOpts := parseHealthCheckOptions(rr, configuration.Backends[frontend.Backend].HealthCheck)
							if hcOpts != nil {
								backendsHealthcheck[frontend.Backend] = healthcheck.NewBackendHealthCheck(*hcOpts)
							}
						}
...
func parseHealthCheckOptions(lb healthcheck.LoadBalancer, hc *types.HealthCheck) *healthcheck.Options {
	if hc == nil || hc.URL == "" {
		return nil
	}
	...
	if hc.Interval != "" {
		...
		switch {
		case err != nil:
			log.Errorf("Illegal healthcheck interval: %s", err)
		case intervalOverride <= 0:
			log.Error("Interval must be larger than zero")
		default:
